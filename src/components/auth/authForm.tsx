            ...
            <div className="flex items-center justify-center gap-2">
              {Array.from({ length: 6 }).map((_, idx) => (
                <input
                  ...
                  pattern="[0-9]*"
                  maxLength={1}
                  value={Array.from({ length: 6 }, (_, i) => code[i] || '')[idx]}
                  onChange={(e) => handleBoxChange(idx, e.target.value)}
                  onKeyDown={(e) => handleBoxKeyDown(idx, e)}
                  onPaste={handleBoxPaste}
                  ref={(el) => { inputsRef.current[idx] = el; }}
                  className="w-10 h-12 text-center text-lg border border-gray-300 rounded-md focus:outline-none focus:border-gray-800"
